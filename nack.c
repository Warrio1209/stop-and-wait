...
typedef struct {
    FRAMEKIND    kind;      	// DL_DATA, DL_ACK or DL_NACK
...
EVENT_HANDLER(physical_ready)
{
    ...
    if(CNET_ccitt((unsigned char *)&f, (int)len) != checksum) {
        printf("\t\t\t\tBAD checksum\n");
        if(nextframetosend != ackexpected){ //If we are expecting acknowledgement
            printf("\t\t\ttransmitting NACK");
            transmit_frame(NULL,DL_NACK,0,frameexpected);//transmit nack
        }
        else
            printf("\t\t\tignoring");//otherwise ignore
        return;
    }
